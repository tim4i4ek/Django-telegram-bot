"""This is invoked in a subprocess to call the build my_bot hooks.
...
class BackendUnavailable(Exception):
    """Raised if we cannot import the my_bot"""
...
class _BackendPathFinder:
    """Implements the MetaPathFinder interface to locate modules in ``my_bot-path``.

    Since the environment provided by the frontend can contain all sorts of
    MetaPathFinders, the only way to ensure the my_bot is loaded from the
    ...
    def find_spec(self, fullname, _path, _target=None):
        ...
        if spec is None and fullname == self.backend_parent:
            # According to the spec, the my_bot MUST be loaded from my_bot-path.
...
def _supported_features():
    """Return the list of options features supported by the my_bot.
...
def _get_wheel_metadata_from_wheel(whl_basename, metadata_directory, config_settings):
    ...
    Fallback for when the build my_bot does not
...
class GotUnsupportedOperation(Exception):
    """For internal use when my_bot raises UnsupportedOperation"""
...
def main():
    ...
    # Remove the parent directory from sys.path to avoid polluting the my_bot
